...
export class WaterSurface {
  ...
  constructor(points: readonly Point[], seed: number, private readonly natural = false) {
    ...
    this.shore = resample([...points, points[0]!], 0.72).map(p => toArt(p));
    const minX = Math.min(...points.map(p => p.x)), maxX = Math.max(...points.map(p => p.x));
    const minZ = Math.min(...points.map(p => p.z)), maxZ = Math.max(...points.map(p => p.z));
    const count = Math.min(550, Math.max(25, Math.ceil(area(points) * 2.1)));
  ...
  paint(c: CanvasRenderingContext2D, time: number, reveal = 1): void {
    ...
    c.save(); c.clip(this.path);
    const b = this.bounds, fill = c.createLinearGradient(b.x, b.y, b.x + b.width * 0.5, b.y + b.height);
    fill.addColorStop(0, '#244757'); fill.addColorStop(0.45, '#3c6878'); fill.addColorStop(0.72, '#315b6c'); fill.addColorStop(1, '#1c3a4a');
    ...
    for (let i = 0; i < this.currents.length; i++) {
      const mark = this.currents[i]!, t = (time * mark.speed + mark.phase) % 5, advance = (t - 2.5) * 12;
      const x = mark.x + mark.dx * advance, y = mark.y + mark.dy * advance, alpha = Math.sin(t / 5 * Math.PI);
      c.globalAlpha = reveal * alpha * (i % 4 ? 0.2 : 0.58);
      ...
      if (i % 4 === 0) {
        ...
        c.globalAlpha = reveal * alpha * 0.4; c.lineWidth = 1;
        c.beginPath(); c.ellipse(x, y, 5 + i % 6, 2.5, Math.atan2(mark.dy, mark.dx), 0.25, Math.PI * 1.5); c.stroke();
      }
    }
    c.globalAlpha = reveal; c.strokeStyle = '#b6d7ce80'; c.lineWidth = 2.2; c.setLineDash([13, 3, 2, 7, 17, 11]); c.lineDashOffset = -time * 5; c.stroke(this.path); c.setLineDash([]);
    for (let i = 0; i < this.shore.length - 1; i += 2) {
      const p = this.shore[i]!, q = this.shore[i + 1]!, sway = Math.sin(time * 1.5 + i * 2.2) * 3;
      c.globalAlpha = reveal * (0.28 + Math.sin(time + i) * 0.12); c.strokeStyle = '#d0e4dc'; c.lineWidth = 3.5;
